...
Note cutadapt which is a dependency of trim galore is not properly configured on hpc, we opt to use fastp instead
```
trim_galore \
    --cores 2 \
...
It is split into a set that should be IUPAC codes and all other bases, using the ConsensusTag in the VCF
```
